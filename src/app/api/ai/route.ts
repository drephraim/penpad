  ...
  try {
    ...
    if (action === "continue") {
      ...
      systemInstruction =
        ...
        "Ensure all requested categories (like Name, Cultivation, Levels, Attributes, Affinity/Element) are mapped to appropriate template cards. For attributes, create a 'stat' or 'compound' card and ensure the used stats are in settings.statKeys and set defaults in profileTemplate.defaultStats. For elemental affinity or bloodline cards, create a compound card with companion fields for ranks/grades, such as ['Spirit Affinity', 'Spirit Affinity Ranks'] or ['Bloodline', 'Bloodline Grade'], and include those fields in settings.customFields."
