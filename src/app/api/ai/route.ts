  ...
  try {
    ...
    if (action === "continue") {
      ...
      systemInstruction =
        ...
        "3. The profile schema is intentionally simple: name, title, bloodline name, bloodline rank, affinity names/ranks, main class, secondary class, skills, and lore.\n" +
        "4. Map cultivation stage names such as Immortal, Demigod, God, God King, Saint, True Immortal, or Mortal to profile.realm. Map sub-ranks such as Low, Medium, High, Peak, Early, Middle, Late, or Perfection to profile.stage. Do not put cultivation sub-ranks in profile.rank unless the story has a separate non-cultivation rank system.\n" +
        "5. Put the character's main class/job/role in profile.className. If the chapter gives a second class, subclass, job, or secondary role, put it in profile.customFields['Secondary Class'].\n" +
        "6. Put the bloodline name in profile.customFields['Bloodline'] and its rank/grade/tier/quality such as Supreme or Celestial in profile.customFields['Bloodline Rank'].\n" +
        "7. Put affinity names in profile.customFields['Affinity Names'] as a comma-separated list, such as Fire, Ice, Void. Put paired affinity ranks in profile.customFields['Affinity Rank'], such as Fire (High), Ice (Celestial), Void (Supreme). Preserve pairings when multiple affinities have different ranks.\n" +
        "8. Put skills, techniques, powers, spells, and signature abilities in profile.abilities. Each ability must include name, rank if known, level set to 1 when no number is stated, description, and evidence when available.\n" +
        "9. Put one interesting reusable lore note in profile.notes: something said about the character that may matter later, such as reputation, prophecy, rumor, weakness, relationship, hidden identity, origin, temperament, or an unusual detail.\n" +
        "10. Preserve existing values when the current chapter gives no direct evidence. If the chapter has no meaningful progression or new character detail, keep the profile stable and set update.shouldApply to false. Still summarize that it was reviewed.\n" +
        "11. Never double-count previous profile history. Use the existing profile and processed chapter history as current truth.\n" +
        "12. Output ONLY valid JSON with keys: targetLoreEntryId, targetProfileId, profile, and update. No markdown fences. profile must include name, title, className, rank, realm, stage, abilities, customFields, notes, level, exp, nextLevelExp, stats, traits, nicknames, uniqueTrait, and cultivationPath. customFields must include Bloodline, Bloodline Rank, Affinity Names, Affinity Rank, and Secondary Class. update must include shouldApply, summary, levelBefore, levelAfter, realmBefore, realmAfter, stageBefore, stageAfter, statChanges, abilityChanges, rewards, and evidence."
